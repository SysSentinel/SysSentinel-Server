...
public class HostToMetricsFrontendController {
    ...
    @GetMapping("/systems")
    public ResponseEntity<Page<SystemEntityPersistent>> sendSystems(@AuthenticationPrincipal Jwt jwt, @RequestHeader("login") String login, @PageableDefault(size = 10) Pageable pageable){
        ...
        ArrayList<SystemEntityPersistent> systems = new ArrayList<>();
    ...
    public ResponseEntity<SystemVolatileEntityPersistent> sendSystemVolatileInfo(@AuthenticationPrincipal Jwt jwt, @RequestHeader("login") String login, @RequestParam String uuid, @RequestParam(defaultValue = "cpuLoad,desc") String sort) throws JsonProcessingException {
        if (jwt == null || login == null) return ResponseEntity.status(401).build();
        if (!jwt.getSubject().equals(login)) return ResponseEntity.status(401).build();
        ...
        SystemVolatileEntityPersistent svedto = sver.getByUUID(uuid);
